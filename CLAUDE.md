...
**Newest (2026-09-26): the GPU trades clock against CPU clock** - see the "GPU clock is traded against CPU clock" trap. Quiet reaches a sustained **2200 MHz** GPU because EPP `power` slows the cores; an all-core CPU cap reproduces it but made CP2077 and RE4 stutter. **Next is phase E** - Atom-only caps, P-cores free:

```
sudo systemctl stop fw-helperd && PHASES=E sudo -E ./scratchpad/gpu-cpu-tradeoff.sh 2>&1 | tee ~/gpu-cpu-tradeoff-E.log; sudo systemctl start fw-helperd
```

Then Shadow of the Tomb Raider (separate CPU/GPU fps) for both this and parking. Before that session:
