...
| M9 — two game profiles + benchmarking | **not started, gated on a measurement.** `game` (stop wasting budget) and `retro` (single-thread, core parking), plus an A/B benchmark loop on the M8 recorder. The circulating blueprint for this laptop was checked path by path (2026-09-22): three of its four mechanisms do not exist here. See `docs/framework_gaming_profile.md` and M9 in `docs/plan.md` |
...
**Start with M9 Phase 0.** Session of 2026-09-22 planned two game profiles and the
benchmark loop that judges them. Nothing is implemented. Read
